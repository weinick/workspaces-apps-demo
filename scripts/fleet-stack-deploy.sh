...
echo ""
echo "下一步 - 预热实例（培训前执行）："
echo "  bash scripts/scale-fleet.sh warmup <count> （REGION=${REGION} ENV_NAME=${CFN_STACK_NAME}-${FLEET_SUFFIX}）"
echo "  REGION=${REGION} ENV_NAME=${CFN_STACK_NAME}-${FLEET_SUFFIX} bash scripts/scale-fleet.sh warmup $MIN_CAPACITY"
echo ""
echo "下一步 - 生成 Streaming URL："
echo "  bash scripts/generate-urls.sh $REGION ${CFN_STACK_NAME}-${FLEET_SUFFIX} <人数> <有效期小时>"
