...
echo ""
echo "下一步 - 预热实例（培训前执行）："
echo "  bash scripts/scale-fleet.sh warmup <count> （ENV_NAME=${CFN_STACK_NAME}-${FLEET_SUFFIX}）"
echo "  ENV_NAME=${CFN_STACK_NAME}-${FLEET_SUFFIX} bash scripts/scale-fleet.sh warmup $MIN_CAPACITY"
echo ""
