...
# 生成环境配置文件，供后续脚本自动加载
ENV_FILE=".env.${CFN_STACK_NAME}-${FLEET_SUFFIX}"
cat > "$ENV_FILE" <<EOF
# Auto-generated by fleet-stack-deploy.sh at $(date -u '+%Y-%m-%d %H:%M:%S UTC')
export REGION=$REGION
export ENV_NAME=${CFN_STACK_NAME}-${FLEET_SUFFIX}
export CFN_STACK_NAME=$CFN_STACK_NAME
export FLEET_SUFFIX=$FLEET_SUFFIX
export CUSTOM_IMAGE_NAME=$CUSTOM_IMAGE_NAME
export FLEET_INSTANCE_TYPE=$FLEET_INSTANCE_TYPE
export FLEET_TYPE=$FLEET_TYPE
EOF
echo "环境配置已保存到: $ENV_FILE"
echo ""
echo "下一步 - 预热实例（培训前执行）："
echo "  source $ENV_FILE && bash scripts/scale-fleet.sh warmup <count>"
echo ""
echo "下一步 - 生成 Streaming URL："
echo "  source $ENV_FILE && bash scripts/generate-urls.sh \$REGION \$ENV_NAME <人数> <有效期小时>"
